...
class AudioProcessor:
    ...
    async def _process_command(self, text: str):
        ...
        # 3. 物品搜索指令
        if "找" in text or "搜索" in text or "在哪" in text:
    ...
    async def _synthesize_speech(self, text: str):
        ...
        """
        try:
            print(f"[TTS] 准备启动子进程合成: {text}")
            
            audio_data = await self._tts_to_wav(text)
    ...
    async def _tts_to_wav(self, text: str) -> bytes:
        """
        通过调用独立的 tts_synthesizer.py 脚本将文本转换为WAV音频数据。
        这种方法可以完全隔离pyttsx3引擎，避免主进程中的状态问题。
        """
        try:
            # 获取Python解释器路径，确保在虚拟环境也能正确运行
            python_executable = sys.executable
            script_path = os.path.join(os.path.dirname(__file__), 'tts_synthesizer.py')
            ...
            # 检查是否有错误输出
            if stderr:
                print(f"[TTS子进程错误]: {stderr.decode('utf-8', errors='ignore').strip()}")

            # 检查进程返回码
            if proc.returncode != 0:
                print(f"[TTS子进程] 异常退出，返回码: {proc.returncode}")
                return b''

            # 从标准输出获取Base64编码的音频数据
            if stdout:
